...
func (tx *Tx) init() {
	...
	tx.ConfigProperty = NewConfigPropertyClient(tx.config)
	tx.EventServiceLog = NewEventServiceLogClient(tx.config)
	tx.INSTALLEDUPGRADES = NewINSTALLEDUPGRADESClient(tx.config)
	tx.LDAPUser = NewLDAPUserClient(tx.config)
	tx.ManagedUser = NewManagedUserClient(tx.config)
	tx.MappedLDAPGroup = NewMappedLDAPGroupClient(tx.config)
	tx.MappedOIDCGroup = NewMappedOIDCGroupClient(tx.config)
	tx.OIDCGroup = NewOIDCGroupClient(tx.config)
	tx.OIDCUser = NewOIDCUserClient(tx.config)
