...
func (akq *APIKeyQuery) QueryTeams() *TeamQuery {
	query := (&TeamClient{config: akq.config}).Query()
...
func (akq *APIKeyQuery) WithTeams(opts ...func(*TeamQuery)) *APIKeyQuery {
	query := (&TeamClient{config: akq.config}).Query()
...
func (akq *APIKeyQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*APIKey, error) {
	...
	if query := akq.withTeams; query != nil {
		if err := akq.loadTeams(ctx, query, nodes,
			func(n *APIKey) { n.Edges.Teams = []*Team{} },
			func(n *APIKey, e *Team) { n.Edges.Teams = append(n.Edges.Teams, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (akq *APIKeyQuery) loadTeams(ctx context.Context, query *TeamQuery, nodes []*APIKey, init func(*APIKey), assign func(*APIKey, *Team)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(apikey.TeamsTable)
		s.Join(joinT).On(s.C(team.FieldID), joinT.C(apikey.TeamsPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(apikey.TeamsPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(apikey.TeamsPrimaryKey[1]))
